    ...
    const fetchEvents = async () => {
      try {
        ...
        const response = await fetch(
          `/api/calendar/events?startDate=${startDate}&endDate=${endDate}&role=student`,
          { credentials: 'include' }
        )
        if (!response.ok) {
              ...
              <div className="space-y-3">
                {monthEvents.map((event) => (
                  <TooltipProvider key={event.id} delayDuration={200}>
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <div className="flex items-start gap-3 py-2 border-b border-border last:border-0 cursor-pointer">
