...
export async function GET(request: NextRequest) {
  try {
    ...
    const averageAttendance = totalClasses > 0
      ...
      : 0
    const hasAttendanceRecords = totalClasses > 0
    ...
    const now = new Date()
    // Window: past 6 months through all upcoming, so the dashboard can group by month
    const windowStart = new Date(now)
    windowStart.setMonth(windowStart.getMonth() - 6)

    const eventRecords = await prisma.calendarEvent.findMany({
      where: {
        startAt: { gte: windowStart },
        OR: [
          { audience: 'ALL' },
          { audience: 'STUDENT' },
          { AND: [{ audience: 'CLASS_ONLY' }, { classId: currentClassId }] },
        ],
      },
      orderBy: { startAt: 'asc' },
      take: 100,
